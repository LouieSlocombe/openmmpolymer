...
def _simulation(
    ...
    """A Simulation with one barostat attached at zero frequency, to be asked."""
    return bare_simulation(
        system,
        topology,
        positions,
        barostat=make_barostat(kind, 300.0, 1.0, 0, 7, **barostat),
    )
...
def test_no_barostat_is_a_message_naming_the_fix() -> None:
    ...
    simulation = bare_simulation(*ideal_gas_system(64, 3.0))
    with pytest.raises(StressError, match="frequency=0"):
        pressure_tensor_bar(simulation)
    with pytest.raises(StressError):
        _require_barostat(simulation)
